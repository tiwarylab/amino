...
def distortion(centers, ops, mut): '''PARALLEL'''
    dis = 0.0
    for i in ops:
        min_val = np.inf
        for j in centers:
...
class DissimilarityMatrix:
    ...
    def add_OP(self, OP): '''PARTIALLY PARALLEL, UPDATE IS SEQUENTIAL'''
...
def d1_bin(x, bins = 80): '''PARALLEL, BINNING CAN BE DONE WITH DIGITIZE'''
...
def d2_bin(x, y, bins = 80): '''PARALLEL, BINNING CAN BE DONE WITH DIGITIZE'''
...
def grouping(new_OPs, all_OPs, mut): '''PARALLEL'''
...
def group_evaluation(OPs, mut):  '''PARALLEL'''
...
def find_ops(old_ops, max_outputs): '''PARTIALLY PARALLEL'''
